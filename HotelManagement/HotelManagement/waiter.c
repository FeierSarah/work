...
void SelectFullRoomByno(char* no)
{
	...
	printf("房间号\t房间类型\t是否满\t是否清扫\t房间号\t姓名\t身份证号\t电话\t入住天数\t入住时间\t搬出时间\n");
...
void clientadd(char* no)
{
	...
	sprintf(sql, "insert into client values('%s','%s','%s','%s','%d','%s','%s')", no, Cname, Cno, Ctele, Cday, Cin, Cout);
